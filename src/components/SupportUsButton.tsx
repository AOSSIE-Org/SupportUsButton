...
function isDarkColor(colorStr?: string): boolean {
  ...
  const lum =
    0.2126 * sRgbLuminance(r) +
    0.7152 * sRgbLuminance(g) +
    0.0722 * sRgbLuminance(b);
  return lum < 0.179;
}
...
function SupportUsButton(props: supportUsButtonProps): React.JSX.Element {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 p-2 sm:p-10 max-w-7xl mx-auto">
          ...
          {showProjectInfo && projectInformation?.name && (
            <div>
              <span className="text-lg sm:text-base md:text-xl leading-snug flex flex-wrap justify-center items-center gap-1">
                <p className="italic">A Project Powered by</p>
                {validatedUrl ? (
                  <a
                    href={validatedUrl}
                    target="_blank"
        ...
        <div
          className="mt-10 flex flex-wrap justify-center gap-6 animate-sub-fade-in"
